...
on first startup with an empty database, the **setup wizard** walks you through initial configuration — deployment mode, HA, subnets, pools, reservations, conflict detection, DNS proxy, and floating VIPs. after that, all database-backed config is managed through the web UI Configuration page or the REST API
...
database-backed config changes take effect immediately — no restart or SIGHUP needed
...
These sections live in the TOML file only and are NOT stored in the database. this is the only config you edit by hand
...
High availability / failover settings. this is a **bootstrap section** — it lives in the TOML config file, not the database. each node needs its own `[ha]` block because each side has a different role and peer address. the web UI and setup wizard can also edit HA config (they write directly to the TOML file). see [high-availability.md](high-availability.md) for full setup guide
...
These sections are stored in the database and managed through the **web UI Configuration page** or **REST API**. initial values are set during the setup wizard on first boot. changes take effect immediately — no restart needed. synced between HA peers automatically

> **duration format** used throughout: Go duration strings like `"500ms"`, `"10s"`, `"1h"`, `"24h"`, `"1h30m"` etc
...
Dynamic blocklists/allowlists for domain blocking
...
**Web UI:** Configuration > Fingerprinting
**API:** `GET/PUT /api/v2/config/fingerprint`
...
**Web UI:** also available at Reservations page (flat view across all subnets)
**API:** `GET/POST/PUT/DELETE /api/v2/reservations`, `POST /api/v2/reservations/import`
...
for anything not covered by the built-in fields
